...
let currentUser = null; let currentMode = ''; let currentSubject = '宿題';
...
function processAndCompleteEnrollment() {
    ...
    // 文字位置修正：学年 134px, 氏名 175px
    ctx.fillText(grade+"年生", 190*2.5, 134*2.5+32); ctx.fillText(name, 190*2.5, 175*2.5+42);
...
document.getElementById('hw-input').addEventListener('change', async (e) => {
    ...
    let p = 0; const timer = setInterval(() => { if (p < 90) { p += 4; bar.style.width = p+'%'; text.innerText = p; } }, 500);
    try {
        ...
        clearInterval(timer); bar.style.width = '100%'; text.innerText = '100';
...
function selectMode(m) { currentMode = m; switchScreen('screen-main'); document.getElementById('upload-controls').classList.remove('hidden'); document.getElementById('problem-selection-view').classList.add('hidden'); document.getElementById('final-view').classList.add('hidden'); updateNellMessage("宿題をみせてにゃ！", "happy"); }
