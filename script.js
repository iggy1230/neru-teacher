...
async function loadFaceModels() {
    ...
    try {
        ...
        modelsLoaded = true;
        const loader = document.getElementById('loading-models');
        if(loader) loader.innerText = "ネル先生の準備OKにゃ！🐾";
...
function processAndCompleteEnrollment() {
    ...
    // 修正：136+3=139 (学年)
    ctx.fillText(grade+"年生", 190*2.5, (136+3)*2.5+32); 
...
document.getElementById('hw-input').addEventListener('change', async (e) => {
    ...
    } catch (err) { updateNellMessage("Google先生が制限エラーだにゃ🐾"); document.getElementById('upload-controls').classList.remove('hidden'); document.getElementById('thinking-view').classList.add('hidden');
...
function renderProblemSelection() {
    ...
    if(transcribedProblems.length === 0) {
        list.innerHTML = "<p>読み取れなかったにゃ。もう一度撮ってみてにゃ。</p>";
        return;
    }
...
function pressThanks() { const today = new Date().toISOString().split('T')[0]; currentUser.attendance[today] = 'red'; currentUser.karikari = (currentUser.karikari || 0) + 5; saveAndSync(); switchScreen('screen-lobby'); }
