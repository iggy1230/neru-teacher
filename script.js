...
// --- 音声合成 (ja-JP-Neural2-B via Server) ---
async function speakNell(text, mood = "normal") {
    if (!text) return;
    if (currentAudio) { currentAudio.pause(); currentAudio = null; }
    try {
        const res = await fetch('/synthesize', { // Render上では相対パスでOKにゃ
...
document.getElementById('hw-input').addEventListener('change', async (e) => {
    ...
    try {
        ...
        transcribedProblems = data.map(pr => {
            ...
            let safeHints = ["問題をゆっくり読んでにゃ","言葉の式を書いてみるにゃ","ネル先生と計算するにゃ！"];
    ...
    } catch (err) { clearInterval(pTimer); updateNellMessage("制限エラーだにゃ🐾"); document.getElementById('upload-controls').classList.remove('hidden');
...
function revealAnswer() { 
    ...
    updateNellMessage(`答えは……「${ans}」だにゃ！……カッコにはこの数字が入るんだよ。`, "gentle"); 
}
