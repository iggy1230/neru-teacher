...
document.getElementById('student-photo-input').addEventListener('change', async (e) => {
    ...
    if (det) {
        ...
        ctx.drawImage(decoEars, (targetW - targetW*0.95)/2, fY - (targetW*0.95 * 0.5), targetW*0.95, targetW*0.95);
    ...
    return new Promise((resolve) => {
        ...
        reader.onload = (e) => {
            const img = new Image(); img.onload = () => {
                ...
                const MAX = 800; // 通信量を極限まで減らすために800pxに制限にゃ
                let w = img.width, h = img.height;
                if (w > MAX || h > MAX) { if (w > h) { h *= MAX / w; w = MAX; } else { w *= MAX / h; h = MAX; } }
                canvas.width = w; canvas.height = h;
                canvas.getContext('2d').drawImage(img, 0, 0, w, h);
                // 画質を0.6まで落としてトークン消費を抑えるにゃ
...
document.getElementById('hw-input').addEventListener('change', async (e) => {
    ...
    const timer = setInterval(() => { if (p < 90) { p += 3; pBar.style.width = p+'%'; pTxt.innerText = p; } }, 400);
    ...
    try {
        ...
        if (res.status === 429) {
            updateNellMessage("Google先生が制限エラーだにゃ🐾 3秒まってリトライするにゃ。", "thinking");
        ...
        if (!res.ok) throw new Error("Google先生がお疲れだにゃ🐾");
...
function renderProblemSelection() {
    ...
    if (!Array.isArray(transcribedProblems)) transcribedProblems = [];
