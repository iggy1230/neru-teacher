...
document.getElementById('student-photo-input').addEventListener('change', async (e) => {
    ...
    if (detection) {
        ...
        let cropW = box.width * 2.8; 
        let cropH = cropW / aspect;
        let sX = box.x + box.width / 2 - cropW / 2;
        let sY = box.y + box.height / 2 - cropH * 0.45;
        
        // 描画
        ctx.drawImage(img, sX, sY, cropW, cropH, 0, 0, targetW, targetH);
        
        const scale = targetW / cropW;
        const nX = (nose.x - sX) * scale;
        const nY = (nose.y - sY) * scale;
        const headTopY = (box.y - sY) * scale; // 顔の枠の上端
        ...
        // マズル：鼻の位置
        const mW = targetW * 0.65;
        ctx.drawImage(decoMuzzle, nX - mW/2, nY - mW/3.5, mW, mW * 0.8);
