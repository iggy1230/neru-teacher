...
document.getElementById('student-photo-input').addEventListener('change', async (e) => {
    ...
    if (detection) {
        ...
        const fY = (box.y - sY) * scale; // 頭のてっぺん付近
        const earW = targetW * 0.95;
        // 猫耳の位置を調整（fYを基準におでこの上へ）
...
function processAndCompleteEnrollment() {
    ...
    ctx.fillText(grade+"年生", 190*2.5, (136+5+2)*2.5+32); // 学年位置
    ctx.fillText(name, 190*2.5, (175+2)*2.5+42); // 氏名位置
