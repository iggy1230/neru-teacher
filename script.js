...
async function loadFaceModels() {
    ...
    try {
        ...
        modelsLoaded = true;
        document.getElementById('loading-models').innerText = "ネル先生の準備OKにゃ！🐾";
...
document.getElementById('student-photo-input').addEventListener('change', async (e) => {
    ...
    if (det) {
        ...
        ctx.drawImage(decoEars, (targetW - targetW*0.95)/2, fY - (targetW*0.95 * 0.45), targetW*0.95, targetW*0.95);
    ...
    return new Promise((resolve) => {
        ...
        reader.onload = (e) => {
            const img = new Image(); img.onload = () => {
                const canvas = document.createElement('canvas'); const MAX = 1200; // 高解像度維持
...
document.getElementById('hw-input').addEventListener('change', async (e) => {
    ...
    let progress = 0; const pBar = document.getElementById('progress-bar'); const pTxt = document.getElementById('progress-percent');
    const timer = setInterval(() => { if (progress < 90) { progress += 3; pBar.style.width = progress+'%'; pTxt.innerText = progress; } }, 400);

    try {
        updateNellMessage("どれどれ……ネル先生がじっくり見てあげるにゃ。……", "thinking");
        const b64 = await shrinkImage(e.target.files[0]);
        const res = await fetch('/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ image: b64, mode: currentMode, grade: currentUser.grade }) });
        const data = await res.json();
        ...
        setTimeout(() => {
            ...
            if (currentMode === 'explain' && transcribedProblems.length > 0) {
                renderProblemSelection();
            } else if (currentMode === 'grade' && transcribedProblems.length > 0) {
                showGradingView();
            } else {
    ...
    } catch (err) { clearInterval(timer); updateNellMessage("制限エラーだにゃ🐾"); document.getElementById('upload-controls').classList.remove('hidden'); document.getElementById('thinking-view').classList.add('hidden');
