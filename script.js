...
document.getElementById('hw-input').addEventListener('change', async (e) => {
    ...
    let p = 0; const pTimer = setInterval(() => { if (p < 90) { p += 3; updateProgress(p); } }, 500);

    try {
        const b64 = await shrinkImage(e.target.files[0]);
        let res = await fetch('/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ image: b64, mode: currentMode, grade: currentUser.grade }) });
        
        if (res.status === 429) {
            updateNellMessage("Google先生が少しお疲れだにゃ🐾 3秒まってリトライするにゃ。", "thinking");
            await new Promise(r => setTimeout(r, 3500));
            res = await fetch('/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ image: b64, mode: currentMode, grade: currentUser.grade }) });
        }
        
        const data = await res.json();
        transcribedProblems = data.map(pr => {
            let labels = ["【考え方】", "【式の作り方】", "【計算のコツ】"];
            let safeHints = ["問題をゆっくり読んでにゃ","式を作ってみよう","計算をがんばるにゃ！"];
    ...
    } catch (err) { updateNellMessage("制限エラーだにゃ🐾 1分おいてからもう一度ためしてみてにゃ。"); document.getElementById('upload-controls').classList.remove('hidden'); document.getElementById('thinking-view').classList.add('hidden');
...
function showHintStep() {
    ...
    if(hintIndex < 2) { next.classList.remove('hidden'); reveal.classList.add('hidden'); } 
    else { next.classList.add('hidden'); reveal.classList.remove('hidden'); }
}
...
async function pressThanks() { await updateNellMessage("よくがんばったにゃ！……ネル先生も嬉しいにゃ！", "happy"); backToProblemSelection(); }
