// --- anlyze.js (完全版 v125.0: ヒント進化 & 記憶連携修正) ---
...
window.startHint = function(id) {
    ...
    if(nextBtn) { 
        ...
        nextBtn.onclick = window.showNextHint; // 引数なしで呼ぶ（globalのselectedProblemを使う）
    }
...
window.showNextHint = function() {
    ...
    if (p.currentHintLevel === 1) {
        ...
        p.currentHintLevel = 1; // 3つ見たら最初に戻る
    }

    updateNellMessage(text, "thinking");
    const hl = document.getElementById('hint-step-label'); 
    if(hl) hl.innerText = `ヒント Lv.${p.currentHintLevel === 1 ? 3 : p.currentHintLevel - 1}`; // 表示中のレベル
    
    // 3つ目のヒントまで見たあとなら答えを見るボタンを出す
    // (レベルが1に戻った＝3を見た後)
...
// ... (以下、renderProblemSelection, showGradingViewなどは v118 と同じ)
...
window.renderProblemSelection = function() { 
    ...
    transcribedProblems.forEach(p => { 
        const div = document.createElement('div'); div.className = "grade-item"; 
        ...
        const currentVal = p.student_answer || "";
        div.innerHTML = `
                ...
                <div style="flex:1; margin-left:10px;">
                    ...
                    <div style="display:flex; justify-content:flex-end; align-items:center; gap:10px; width:100%;">
                        <div style="flex:1;">
                            ...
                            <input type="text" value="${currentVal}" style="width:100%; padding:8px; border:2px solid #f0f0f0; border-radius:8px; font-size:0.9rem; color:#555; font-weight:bold;">
                        </div>
...
window.showGradingView = function(silent = false) { 
    ...
    transcribedProblems.forEach(p => { 
        ...
        const mark = isCorrect ? "⭕" : "❌"; const markColor = isCorrect ? "#ff5252" : "#4a90e2"; const bgStyle = isCorrect ? "background:#fff5f5;" : "background:#f0f8ff;"; const currentVal = p.student_answer || "";
                ...
                <div style="flex:1; margin-left:10px;">
                    ...
                    <div style="display:flex; gap:10px; font-size:0.9rem; align-items:center; width:100%;">
                        <div style="flex:1;">
                            ...
                            <input type="text" value="${currentVal}" oninput="checkAnswerDynamically(${p.id}, this)" style="width:100%; padding:8px; border:2px solid #ddd; border-radius:8px; font-size:1rem; font-weight:bold; color:#333;">
                        </div>
                        <div style="width:80px; text-align:right; flex-shrink:0;">
                            <button class="mini-teach-btn" onclick="startHint(${p.id})">教えて</button>
                        </div>
...
window.checkAnswerDynamically = function(id, inputElem) { 
    const newVal = inputElem.value; const problem = transcribedProblems.find(p => p.id === id); if (!problem) return; 
    problem.student_answer = String(newVal); const normalizedStudent = String(newVal).trim(); const normalizedCorrect = String(problem.correct_answer || "").trim(); const isCorrect = (normalizedStudent !== "") && (normalizedStudent === normalizedCorrect); problem.is_correct = isCorrect; 
    const container = document.getElementById(`grade-item-${id}`); const markElem = document.getElementById(`mark-${id}`); if (container && markElem) { if (isCorrect) { markElem.innerText = "⭕"; markElem.style.color = "#ff5252"; container.style.backgroundColor = "#fff5f5"; } else { markElem.innerText = "❌"; markElem.style.color = "#4a90e2"; container.style.backgroundColor = "#f0f8ff"; } } updateGradingMessage(); 
};
...
window.backToProblemSelection = function() {
    ...
    if (currentMode === 'grade') showGradingView(); else { renderProblemSelection(); updateNellMessage("他も見るにゃ？", "normal"); }
