...
function startMouthAnimation() {
    ...
    setInterval(() => {
        ...
        if (base === 'nell-normal.png') talk = 'nell-talk.png';
...
function selectMode(m) {
    if (currentMode === 'chat') finishChatSession(); // 記憶保存
    currentMode = m; 
    ...
    if (m === 'chat') {
        ...
        // 記憶があれば挨拶を変える
        const greeting = currentUser && currentUser.memory ? `また会えたにゃ！${currentUser.memory}` : "「おはなしする」を押してね！";
...
async function finishChatSession() {
    ...
    try {
        ...
        const res = await fetch('/summarize', {
            ...
            body: JSON.stringify({ history: chatLog.slice(-10) }) // 直近10件
        });
...
async function startLiveChat() {
    ...
    try {
        ...
        // ★記憶と学年を送る
        const gradeParam = currentUser ? currentUser.grade : "1";
        const memParam = currentUser && currentUser.memory ? encodeURIComponent(currentUser.memory) : "";
        liveSocket = new WebSocket(`${wsProtocol}//${window.location.host}?grade=${gradeParam}&memory=${memParam}`);
...
async function startMicrophone() {
    try {
        ...
        workletNode.port.onmessage = (event) => {
            ...
            // ログ記録（ユーザーの発話ありとみなす）
            // ※リアルタイム音声なので正確なテキスト化はクライアントでは難しいが、発話があったことは記録できる
            
            // 音量ビジュアライザー
            let sum = 0; for(let i=0; i<inputData.length; i++) sum += inputData[i] * inputData[i];
            const volume = Math.sqrt(sum / inputData.length);
            const btn = document.getElementById('mic-btn');
            if (btn && volume > 0.02) { btn.style.boxShadow = "0 0 15px #ffeb3b"; btn.style.transform = "scale(1.05)"; } else if(btn) { btn.style.boxShadow = "none"; btn.style.transform = "scale(1)"; }
...
function playPcmAudio(base64) { 
    ...
    const binary = window.atob(base64); const bytes = new Uint8Array(binary.length); for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i); const float32 = new Float32Array(bytes.length / 2); const view = new DataView(bytes.buffer); for (let i = 0; i < float32.length; i++) float32[i] = view.getInt16(i * 2, true) / 32768.0; 
...
document.getElementById('hw-input').addEventListener('change', async (e) => {
    ...
    try {
        ...
        setTimeout(() => { 
            ...
            else { showGradingView(); /*報酬ロジック等は既存*/ }
        }, 800);
