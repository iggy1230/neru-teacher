// --- anlyze.js (Live API Text & Audio - Final Version) ---
...
// 2. ★Live Chat (Live API with Text & Audio)
        ...
        liveSocket.onmessage = async (event) => {
            ...
            if (data.serverContent?.modelTurn?.parts) {
                ...
                for (const part of parts) {
                    // ★ SHOW TEXT IN BUBBLE (Without triggering standard TTS)
                    if (part.text) {
                        const el = document.getElementById('nell-text');
                        if (el) el.innerText = part.text;
                    }
                    // PLAY AUDIO
                    if (part.inlineData) {
...
async function startMicrophone() {
    try {
        ...
        workletNode.port.onmessage = (event) => {
            ...
            if (Math.sqrt(sum / inputData.length) > 0.02 && btn) {
