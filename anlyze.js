...
// 効果音 (エラー回避のためtry-catchで生成)
let sfxBori, sfxHit, sfxPaddle, sfxOver, sfxBunseki, bgmApp;
try {
    sfxBori = new Audio('boribori.mp3');
    sfxHit = new Audio('cat1c.mp3');
    sfxPaddle = new Audio('poka02.mp3'); 
    sfxOver = new Audio('gameover.mp3');
    sfxBunseki = new Audio('bunseki.mp3'); sfxBunseki.volume = 0.1; sfxBunseki.loop = true;
    bgmApp = new Audio('bgm.mp3'); bgmApp.loop = true; bgmApp.volume = 0.2;
} catch(e) { console.log("Audio init error (ignored)"); }
    ...
    if (currentUser.isGoogleUser && typeof db !== 'undefined') {
        try {
            await db.collection("memories").doc(currentUser.id).set({ 
                history: firebase.firestore.FieldValue.arrayUnion(newItem),
...
window.updateNellMessage = async function(t, mood = "normal") {
    const el = document.getElementById('nell-text') || document.getElementById('nell-text-game');
    if (el) el.innerText = t;

    if (t && t.includes("もぐもぐ") && sfxBori) { try { sfxBori.currentTime = 0; sfxBori.play(); } catch(e){} }
    if (!t || t.includes("ちょっと待ってて") || t.includes("もぐもぐ")) return;

    saveToNellMemory('nell', t);

    if (typeof speakNell === 'function') {
        await speakNell(t.replace(/🐾/g, ""), mood);
    }
};
...
window.selectMode = function(m) {
    ...
    if (typeof switchScreen === 'function') {
        ...
        alert("エラー：画面切り替え機能が見つかりません。リロードしてください。");
        return;
    }
...
window.giveLunch = function() {
    ...
    .then(r => r.json())
    .then(d => { setTimeout(() => { updateNellMessage(d.reply || "おいしいにゃ！", d.isSpecial ? "excited" : "happy"); }, 1500); })
...
window.pressAllSolved = function(btnElement) { 
    ...
    if (currentUser) { currentUser.karikari += 100; saveAndSync(); showKarikariEffect(100); updateMiniKarikari(); 
        updateNellMessage("よくがんばったにゃ！カリカリ100個あげるにゃ！", "excited").then(() => { setTimeout(() => { if(typeof window.backToLobby === 'function') window.backToLobby(true); }, 3000); });
    }
};
...
async function playAnalyzeSequence(firstMessage) {
    const messages = [
        { text: firstMessage, mood: "thinking" },
...
function initCustomCropper() {
    ...
    document.getElementById('cropper-cancel-btn').onclick = () => { modal.classList.add('hidden'); window.onmousemove = null; window.ontouchmove = null; document.getElementById('upload-controls').classList.remove('hidden'); isAnalyzing = false; };
...
async function startAnalysis(b64) {
    ...
    try {
        ...
        if (!data || data.length === 0) { 
            cleanupAnalysis(timer);
            updateNellMessage("問題が見つからなかったにゃ…\nもう一度写真を撮ってみて！", "thinking"); 
            setTimeout(() => { document.getElementById('thinking-view').classList.add('hidden'); document.getElementById('upload-controls').classList.remove('hidden'); if(backBtn) backBtn.classList.remove('hidden'); }, 3000); 
            return; 
        }
...
function cleanupAnalysis(timerId) { isAnalyzing = false; if(timerId) clearInterval(timerId); try{sfxBunseki.pause()}catch(e){} }