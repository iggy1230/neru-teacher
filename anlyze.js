// --- anlyze.js (完全版 v226.0: タグ解析ロジック強化版) ---
...
async function addToNellCollection(name) {
    if (!window.NellMemory || !currentUser || !lastSnapshotCanvas) return;
    
    console.log(`【Collection】図鑑登録処理: ${name}`);
...
window.updateNellMessage = async function(t, mood = "normal", saveToMemory = false, speak = true) {
    ...
    // ★重要: タグ除去の強化 (display, save, スペース対応)
    const displayText = t.replace(/(?:\[|【)\s*(?:DISPLAY|SAVE|save)[:：]\s*(.+?)\s*(?:\]|】)/gi, "");
