// --- anlyze.js (完全版 v23.0: 音声一本化・ボタン修正) ---
...
// 口パク（グローバル変数 isNellSpeaking を監視）
function startMouthAnimation() {
    ...
    setInterval(() => {
        ...
        // window.isNellSpeaking は audio.js または LiveChat で制御される
...
async function updateNellMessage(t, mood = "normal") {
    ...
    // 3. ★音声再生 (audio.js の speakNell に一任する！)
    if (typeof speakNell === 'function') {
        await speakNell(t, mood);
    } else {
        console.error("speakNell function not found in audio.js");
    }
}
...
window.setSubject = function(s) { 
    currentSubject = s; 
    // ※ saveAndSyncはuser.jsにあるが、ここから呼べない場合はスキップ
...
async function startAnalysis(b64) {
    ...
    try {
        ...
        setTimeout(() => { 
            ...
            // 最初の問題文を読み上げる (修正点)
            const firstQ = data[0] ? data[0].question : "読めたにゃ！";
            
            if (currentMode === 'explain' || currentMode === 'review') { 
                renderProblemSelection(); 
                updateNellMessage(firstQ, "normal"); 
            } else { 
                showGradingView(); 
                updateNellMessage(firstQ, "normal"); 
            }
...
function showNextHint() {
    ...
    if(typeof saveAndSync==='function') saveAndSync(); 
    updateMiniKarikari(); showKarikariEffect(-cost);
