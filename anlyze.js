...
// ※ audio.js と連携するために window.isNellSpeaking を参照・操作します
...
function startMouthAnimation() {
    ...
    setInterval(() => {
        ...
        // グローバル変数 isNellSpeaking が true なら口パク
        if (window.isNellSpeaking) {
            toggle = !toggle;
            if (toggle) {
                img.src = talkIcon; // 口を開ける
            } else {
                // 口を閉じる（教科画像があればそれ、なければデフォルト）
            ...
            // しゃべっていない時は口を閉じる
            // (nell-talk.png のまま止まらないように戻す)
            if (img.src.includes('nell-talk')) {
                if (currentSubject && subjectImages[currentSubject] && currentMode !== 'chat' && currentMode !== 'lunch') {
                    img.src = subjectImages[currentSubject];
                } else {
                    img.src = defaultIcon;
...
document.getElementById('hw-input').addEventListener('change', async (e) => {
    ...
    const upCtrl = document.getElementById('upload-controls');
    if(upCtrl) upCtrl.classList.add('hidden');
    
    const thinkView = document.getElementById('thinking-view');
    if(thinkView) thinkView.classList.remove('hidden');
    ...
    try {
        ...
        setTimeout(() => { 
            if(thinkView) thinkView.classList.add('hidden'); 
        ...
        clearInterval(timer); 
        if(thinkView) thinkView.classList.add('hidden'); 
        if(upCtrl) upCtrl.classList.remove('hidden'); 
...
function startHint(id) {
    ...
    const hintView = document.getElementById('hint-detail-container');
    const board = document.getElementById('chalkboard');
    
    if(finalView) finalView.classList.remove('hidden');
    if(hintView) hintView.classList.remove('hidden');
    if(board) {
        board.innerText = selectedProblem.question;
        board.classList.remove('hidden');
    }

    hintIndex = 0; 
    updateNellMessage("カリカリをくれたらヒントを出してあげてもいいにゃ🐾", "thinking"); 
    const hintLabel = document.getElementById('hint-step-label');
    if(hintLabel) hintLabel.innerText = "考え中...";
...
function showNextHint() {
    ...
    const hintLabel = document.getElementById('hint-step-label');
    if(hintLabel) hintLabel.innerText = `ヒント ${hintIndex + 1}`; 
    hintIndex++; 
    
    const nextBtn = document.getElementById('next-hint-btn'); 
    const revealBtn = document.getElementById('reveal-answer-btn');
    
    if (hintIndex === 1 && nextBtn) nextBtn.innerText = "🍖 さらにカリカリを5個あげてヒントをもらう";
    else if (hintIndex === 2 && nextBtn) nextBtn.innerText = "🍖 さらにカリカリを10個あげてヒントをもらう";
    else { 
...
function updateAns(idx, val) {
    ...
    if (normalize(val) === normalize(itm.correct_answer) && val !== "") {
        itm.status = 'correct'; updateNellMessage("正解にゃ！修正ありがとうにゃ。", "happy");
        if (currentUser.mistakes) currentUser.mistakes = currentUser.mistakes.filter(m => m.question !== itm.question);
    } else {
        itm.status = 'incorrect'; updateNellMessage("まだ違うみたいだにゃ……", "thinking");
...
function setSubject(s) { currentSubject = s; if(currentUser){currentUser.history[s]=(currentUser.history[s]||0)+1; saveAndSync();} const icon = document.querySelector('.nell-avatar-wrap img'); if(icon&&subjectImages[s]){icon.src=subjectImages[s];icon.onerror=()=>{icon.src=defaultIcon;};} const sel=document.getElementById('subject-selection-view'); if(sel)sel.classList.add('hidden'); const up=document.getElementById('upload-controls'); if(up)up.classList.remove('hidden'); updateNellMessage(`${currentSubject}の問題をみせてにゃ！`, "happy"); }
