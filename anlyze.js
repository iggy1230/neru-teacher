...
function startMouthAnimation() {
    ...
    setInterval(() => {
        ...
        let talk = talkIcon;
        if (base !== defaultIcon) talk = base.replace('.png', '-talk.png');
...
function fetchGameComment(type, score=0) {
    const now = Date.now();
    // 開始・終了以外は、喋っている途中や3秒以内ならスキップ
    if (type !== 'start' && type !== 'end') {
        if (window.isNellSpeaking || now - lastCommentTime < 3000) return;
    }
    lastCommentTime = now;
    ...
    fetch('/game-reaction', {
        ...
        // audio.jsのspeakNellは既存音声をキャンセルして再生するので、
        // 優先度の高いstart/endはそのまま再生される
...
function drawGame() {
    ...
    ctx.font = "20px serif"; bricks.forEach(b => { if(b.status === 1) ctx.fillText("🍖", b.x + 10, b.y + 20); });
    ...
    if(ball.y+ball.dy < ball.r) ball.dy *= -1;
        ...
        if(ball.x > paddle.x && ball.x < paddle.x + paddle.w) {
            ...
            // 実況: セーブ
            fetchGameComment("save");
        } else if(ball.y+ball.dy > gameCanvas.height-ball.r) {
            endGame(false); return;
        }
    } else if (ball.y > gameCanvas.height - 100 && ball.dy > 0 && Math.random() > 0.6) {
        // 実況: ピンチ
        fetchGameComment("pinch");
    }
