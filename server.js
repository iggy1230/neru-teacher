// --- server.js (完全版 v167.0: グリッド計算アルゴリズム強化 & 既存機能維持) ---
    ...
    try {
        ...
        const ocrRules = {
            'さんすう': `
                ・数式、筆算の配置を正確に読み取る。
                ・**【最重要】方眼（グリッド）の面積計算**:
                  図形が方眼紙の上に描かれている場合、以下の手順で厳密に計算してください。
                  1. **完全な正方形のマス**（塗りつぶされている部分）を1つずつ数える。
                  2. **斜めに半分になった三角形のマス**を数える。
                  3. **「三角形の数 ÷ 2」** を計算し、正方形の数に足す。
                  例: 正方形4個、三角形4個の場合 -> 4 + (4÷2) = 6cm²
        ...
        - 手書きのメモ（長さ、角度など）は「問題の条件」として読み取ってください。
