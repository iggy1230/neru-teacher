// --- server.js (完全版 v171.0: 距離判定強化 & 2秒判定対応) ---
...
app.post('/analyze', async (req, res) => {
    try {
        ...
        const ocrRules = {
            ...
            'こくご': `
                ・**【縦書き・座標厳守】**:
                  設問番号（問一など）と解答欄（□）の紐付けは、**物理的な距離が最も近いもの（直下またはすぐ左）**を厳密に選んでください。
                  縦書きでは行が分かれています。**隣の行（隣の問題）の解答欄を、現在の問題の答えとして誤認しない**ように、行ごとのブロックを意識してください。
        ...
        - **【位置関係の厳守】**: 設問と解答欄はセットです。**距離が離れている解答欄（隣の問題のもの）を誤って読み込まない**ように、幾何学的な配置を正確に分析してください。
