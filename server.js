...
app.use(cors());
app.use(express.json({ limit: '50mb' })); // 画像送信用にサイズ制限を緩和
app.use(express.static(path.join(__dirname, '.')));

// --- API設定 (Render等の環境変数を読み込み) ---
...
function createSSML(text, mood) {
    ...
    const processedText = text
        .replace(/……/g, '<break time="650ms"/>') // ためを作る
        .replace(/にゃ/g, '<prosody pitch="+3st">にゃ</prosody>'); // 語尾を可愛く
...
app.post('/synthesize', async (req, res) => {
    try {
        ...
        const [response] = await ttsClient.synthesizeSpeech({
            ...
            voice: { languageCode: 'ja-JP', name: 'ja-JP-Neural2-B' }, // 自然な日本語音声
...
app.post('/analyze', async (req, res) => {
    try {
        ...
        // 共通のヒント作成指示 (採点モードでもヒントを出せるようにするため)
        ...
        res.status(500).json({ error: "AI解析エラーにゃ" }); 
    }
});
