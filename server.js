// --- server.js (完全版 v27.0: 読み取り復元 + 会話維持) ---
...
app.post('/lunch-reaction', async (req, res) => {
    try {
        ...
        if (isSpecial) {
            ...
            テーマ「${themes[Math.floor(Math.random() * themes.length)]}」について、15文字以内の一言で感想を言ってください。
            語尾は「にゃ」。
            `;
        }
        const result = await model.generateContent(prompt);
...
// --- Live API Proxy (WebSocket: v22.0 キャメルケース版を維持) ---
...
wss.on('connection', async (clientWs, req) => {
    ...
    try {
        ...
        geminiWs.on('open', () => {
            ...
            // ★重要: Aoedeボイス設定 & 正しいキャメルケース (この部分は最新の修正を維持)
            const setupMsg = {
                setup: {
