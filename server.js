// --- server.js (完全版 v159.0: 知識封印 & 領域分離強化 & 画像前処理対応) ---
...
app.post('/analyze', async (req, res) => {
    try {
        ...
        const ocrRules = {
            ...
            'こくご': `
                ・【縦書きレイアウト厳守】
                  画像は鮮明化処理されています。**罫線や余白**を基準に、問題ごとのブロックを厳密に区切ってください。
                  「問一」の答えは、「問一のブロック内」にある手書き文字だけです。「問二」のエリアにある文字を混ぜないでください。
                ・選択肢の順序が入れ替わらないよう、上から下、右から左の座標関係を慎重にトレースしてください。
                ・解答欄（□や括弧）内に筆跡がない場合は、絶対に空文字""にすること。`,
            'りか': `
                ・図や表と設問の位置関係を把握。
                ・「2つ選びなさい」等の問題で正解が複数の場合は、"ア,イ"のようにカンマ区切り。
                ・解答欄に手書きの筆跡が確実に見えない場合は、正解が分かっても絶対に空欄（""）とすること。`,
            'しゃかい': `
                ・【知識封印】あなたの一般的知識（自衛隊など）を使って回答することを**固く禁じます**。
                ・答えは必ず**画像内の図・グラフ・資料の中に書かれている用語**（例: 自主防災隊、消防団など）を使用してください。
                ・資料に書いてある通りの名称で答えることが正解です。
        ...
        - **【位置関係】**: 問題番号と解答欄が離れている場合、幾何学的に最も近いセットを認識してください。隣の問題の答えを誤読しないこと。

        【タスク3: 正解データの作成】
        - **【重要】表記ゆれ（別解）**: 漢字の答えでひらがなも正解とする場合などは、**縦棒 "|" で区切って**併記してください。(例: "高い|たかい")
        - **【重要】複数回答**: 正解が複数の場合は、**カンマ "," で区切って**記述してください。(例: "ア,イ")
        - 社会科は、画像内の資料にある用語を最優先し、一般的知識による回答は避けてください。
...
app.post('/game-reaction', async (req, res) => {
    try {
        ...
        if (type === 'start') {
            ...
            prompt = `あなたはネル先生。ゲーム終了。「${name}さん」のスコアは${score}点（満点20点）。
            必ず「${name}さん」と呼んでください。呼び捨て禁止。
            スコアに応じて20文字以内でコメントして。0-5点:笑って励ます。6-15点:褒める。16点以上:大絶賛。語尾は「にゃ」。`;
        } else {
