// --- server.js (完全版 v163.0: 創造性ゼロ・安定動作版) ---
...
app.post('/analyze', async (req, res) => {
    try {
        ...
        const model = genAI.getGenerativeModel({ 
            ...
            // ★修正: temperature: 0 にしてランダム性を排除（毎回同じ正確な結果を出す）
            generationConfig: { 
        ...
        const ocrRules = {
            ...
            'こくご': `
                ...
                ・隣り合う問題の文章が混ざらないよう、問題ごとの境界線を強く意識する。
                ・選択肢（ア、イ）の内容も問題文に含める。
        ...
        - **【重要】レイアウト認識**: 問題と問題の間には余白や罫線があります。それを基準に、隣の問題の文章が混ざらないように厳密に区別してください。

        【タスク2: 手書き答えの読み取り (OCR & 空欄判定)】
        - ${name}さんが書いた「手書きの答え」を読み取ってください。
        - **【絶対厳守】空欄判定**: 解答欄に**手書きの筆跡がない場合**は、正解が分かっていても**絶対に空文字 ""** にしてください。

        【タスク3: 正解データの作成 (表記ゆれ網羅)】
        - その問題の正しい答えを導き出してください。
        - **【重要】表記ゆれの網羅**: 
          小学生のテストのため、**「漢字」「ひらがな」「交ぜ書き」の全て**を正解とします。
          これらを**縦棒 "|" で区切ってすべて列挙**してください。
          例: 正解が「連携」の場合 -> correct_answer: "連携|れんけい|連けい"
          例: 正解が「自衛隊」の場合 -> correct_answer: "自衛隊|じえいたい|自えい隊"
        - **【重要】複数回答**: 
          正解が複数の場合は、**カンマ "," で区切って**記述してください。(例: "ア,イ")
        ...
        [
          {
            ...
            "label": "①",
            "question": "問題文 (選択肢含む)",
            "correct_answer": "正解 (表記ゆれは|、複数は,で区切る)",
