// --- server.js (完全版 v76.0: 正答率向上・ヒントバレ防止強化) ---
...
app.post('/game-reaction', async (req, res) => {
    try {
        ...
        if (type === 'start') prompt = `あなたはネル先生。生徒「${name}」がゲーム開始。「${name}さん！カリカリいっぱいゲットしてにゃ！」とだけ言って。`;
            ...
            prompt = `あなたはネル先生。ゲーム終了。スコアは${score}個（最大20個）です。${scoreCommentType}内容で、20文字以内でコメントしてください。語尾は「にゃ」。`;
        }
...
app.post('/lunch-reaction', async (req, res) => {
    try {
        ...
        if (count % 10 === 0) {
            ...
            let nameRule = shouldCallName ? `名前「${name}さん」を呼んでください（呼び捨て厳禁）。` : `名前は呼ばないで。`;
            prompt = `ネル先生です。生徒「${name}」さんから給食をもらいました。${nameRule} テーマ「${theme}」について一言感想を言って。語尾「にゃ」。`;
        }
        const result = await model.generateContent(prompt);
    ...
    try {
        ...
        const rules = {
            ...
            'こくご': {
                ...
                hints: `
                  ・【重要：答えバレ厳禁】漢字書き取り問題のヒントでは、その漢字自体（答えの文字）を絶対に使わないこと。「その漢字」や「答えの字」と言い換えること。
        ...
        const prompt = `
            ...
            【★最重要：正答の精度】
            1. 提示された問題に対して、あなたは世界最高の教師として振る舞い、**絶対に間違った答えを出力しないでください。**
            2. 複数の解法を検討し、検証を重ねてから "correct_answer" を決定してください。
            3. 漢字の書き取り問題では、その漢字が本当に正しいか辞書レベルで確認してください。

            【★最重要：ヒント生成ルール（答えバレ厳禁）】
            1. **ヒントの文章の中に、答えそのもの（正解の漢字、用語、数値）を含めることは絶対に禁止です。**
            2. 例：「『箱』という漢字は...」→ NG（答えを書いている）
            3. 例：「竹かんむりに...」→ OK（特徴を言っている）
            4. 以下の3段階のヒントを作成してください。
            ${r.hints}
