// --- server.js (完全版 v168.0: 算数単位なし許容 & 既存機能維持) ---
...
app.post('/analyze', async (req, res) => {
    try {
        ...
        // ★修正: 算数の正解データに単位なしバージョンを含めるよう指示
        const ocrRules = {
            'さんすう': `
                ・数式、筆算の配置を正確に読み取る。
                ・**【正解データの作成】**: 答えが単位を含む場合（例: 5cm²）、**単位なしの数字のみ（例: 5）も正解として認め、縦棒 "|" で区切って併記**してください。(例: "5cm²|5")
                ・方眼（グリッド）の面積計算は、正方形と三角形の数を正確に数えて計算すること。
        ...
        1. **【別解・表記ゆれ】は 縦棒 "|"**
           - 同じ意味で、書き方が違うだけの場合。
           - **算数の単位なし（数字のみ）**もここに含めること。
           - 例: "自主防災隊|じしゅぼうさいたい"
           - 例: "6cm²|6" 

        2. **【複数回答】は カンマ ","**
           - 解答欄が物理的に2つ以上ある場合のみ使用。
           - 例: "ア,イ"
