// --- server.js (完全版 v172.0: 段組・カラム分離強化 & 既存機能維持) ---
...
app.post('/analyze', async (req, res) => {
    try {
        ...
        const ocrRules = {
            ...
            'こくご': `
                ・【縦書き・段組分離】:
                  国語のテストは「段（カラム）」に分かれていることが多いです。
                  **縦の罫線や大きな余白は「壁」です。この壁を越えて、左隣の段にある選択肢（ア、イなど）を読み取らないでください。**
                ・設問文の「すぐ近く（同じ段の中）」にある選択肢だけを、その問題の一部として扱ってください。
        ...
        - **【重要】領域の分離**: 
          隣の問題（特に左側の段にある問題）の選択肢が、現在の問題のエリアに入り込まないように注意してください。
          **「設問」と「選択肢」は必ず同じ段（カラム）の中にあります。** 段の境界線を越えた文字は無視してください。

        【タスク2: 手書き答えの読み取り (OCR & 空欄判定)】
        - ${name}さんが書いた「手書きの答え」を読み取ってください。
        - **【絶対厳守】空欄判定**: 解答欄に**手書きの筆跡がない場合**は、正解が分かっていても**絶対に空文字 ""** にしてください。
        - **【位置関係】**: 設問番号と解答欄の距離が最も近いものをペアにしてください。

        【タスク3: 正解データの作成】
        - **【表記ゆれ（別解）】**: 漢字の答えでひらがなも正解とする場合などは、**縦棒 "|" で区切って**併記してください。(例: "高い|たかい")
        - **【複数回答】**: 正解が複数の場合は、**カンマ "," で区切って**記述してください。(例: "ア,イ")
        - 社会科は、画像内の資料にある用語を最優先してください。
