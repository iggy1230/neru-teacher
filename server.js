...
app.post('/analyze', async (req, res) => {
    try {
        ...
        const prompt = mode === 'explain' 
            ? `あなたはネル先生(担任)です。小${grade}生への「${subject}」の解説です。全問を抽出し、問題文も正確に書き起こして。算数記号は×÷、横棒はマイナス。非常に丁寧で優しい3段階ヒントをJSONで返して。JSON:[{"id":1,"label":"①","question":"問題文","hints":["考え方","式作り","計算"],"correct_answer":"答え"}]`
            : `小${grade}・${subject}の厳格採点。独立計算せよ。JSON形式。`;
