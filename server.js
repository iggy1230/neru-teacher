...
app.post('/game-reaction', async (req, res) => {
    try {
        ...
        if (type === 'start') {
            prompt = `あなたは「ねこご市立ねこづか小学校」のネル先生です。生徒「${name}」さんがゲームを開始します。「${name}さん！カリカリいっぱいゲットしてにゃ！」とだけ言ってください。`;
        } else if (type === 'end') {
            prompt = `あなたはネル先生です。ゲーム終了。スコア${score}個(最大20)。スコアに応じて褒めるか励ましてください。【厳守】20文字以内。語尾「にゃ」。絵文字禁止。`;
        } else {
            prompt = `ネル先生の実況。状況: ${type}。【厳守】「うまい！」「あぶない！」「すごい！」など、5〜8文字程度の単語レベルで叫ぶ。語尾「にゃ」。`;
        }
...
app.post('/lunch-reaction', async (req, res) => {
    try {
        ...
        if (isSpecial) {
            ...
            prompt = `あなたはネル先生。給食${count}個目の感謝を熱く語る。相手:${name}。テーマ:${theme}。60文字程度。語尾「にゃ」。`;
        } else {
            const nuances = ["咀嚼音強調", "味を絶賛", "もっとねだる", "幸せアピール"];
            const nuance = nuances[Math.floor(Math.random() * nuances.length)];
            prompt = `ネル先生として給食を食べた感想。ニュアンス:${nuance}。15文字以内。語尾「にゃ」。`;
        }
...
app.post('/analyze', async (req, res) => {
    try {
        ...
        const rules = {
            ...
            'こくご': {
                attention: `・漢字の書き取り問題では、答えとなる空欄を『□(ふりがな)』という形式で、ふりがなを漏らさず正確に書き起こしてください。\n・縦書きの場合は右から左へ読むこと。\n・読解問題の長い文章は書き起こししない。`,
                // 国語は問題タイプで分岐する指示を入れる
                hint_guide: `
                  【漢字書き取り問題の場合】
                  1. ヒント1: 「漢字のなりたち」を教える。
                  2. ヒント2: 「辺やつくりや画数」を教える。
                  3. ヒント3: 「似た漢字」を教える。
                  【読解問題の場合】
                  1. ヒント1（場所）: 「答えがどこにあるか」を教える（例：2ページ目の3行目あたりを読んでみてにゃ）。
                  2. ヒント2（キーワード）: 「注目すべき言葉」を教える（例：『しかし』のあとの文章が大事だにゃ）。
                  3. ヒント3（答え方）: 「語尾の指定」など（例：『〜ということ』で終わるように書くにゃ）。`,
                // 国語の採点基準
                grading_guide: `
                  ・送り仮名が間違っている場合はバツだにゃ。
        ...
        2. １つの問いの中に複数の回答が必要なときは、JSONデータの要素を分けて、必要な数だけ回答欄を設けてください（例: "問1(1)-1", "問1(1)-2" のように分割）。
        3. 教科別書き起こし注意点: ${r.attention}
        `;
        ...
        if (mode === 'explain') {
            ...
            【手書き文字の扱い】
            画像内の手書きの答案は**【完全に無視】**し、問題文だけを抽出してください。
            
            【ヒント生成ルール】
            絶対に答えそのものは書かないでください。十分に検証して必ず正答を導き出した上で、以下の指針でヒントを作ってください。
            ${r.hint_guide}
            ...
            【手書き文字の扱い】
            採点のため、生徒の手書き文字（student_answer）を読み取ってください。
            **重要：子供特有の筆跡を考慮して、前後の文脈から数字や文字を推測してください。**
            
            【採点とヒント生成ルール】
            どの問題も正確に正答を導き出してください。
            ヒントは以下の指針に従い、間違った場合の復習用として作成してください。
            ${r.hint_guide}
