// --- server.js (完全版 v165.0: 手書きメモ対応・図形認識強化 & 既存機能維持) ---
...
app.post('/analyze', async (req, res) => {
    try {
        ...
        // ★修正: 図形やグラフへの書き込みを「条件」として認識させるルール追加
        const ocrRules = {
            'さんすう': `
                ・数式、筆算の配置を正確に読み取る。
                ・**【重要】手書きメモの認識**: 図形の辺や角の近くに書き込まれた手書きの数字（例: "3cm", "60°"）は、**生徒の答えではなく、問題を解くための「条件（長さや角度）」として認識**し、計算に使用してください。
                ・解答欄（「答え」の横や枠内）にある数字だけを、student_answer として抽出してください。
            ...
            'りか': `
                ・図や表と設問の位置関係を把握。
                ・**【重要】実験結果のメモ**: グラフや表に手書きで数値が書き込まれている場合、それは**実験データ（条件）**として認識し、考察に使用してください。
