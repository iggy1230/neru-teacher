// --- analyze.js (完全版 v291.0: カリカリ更新関数参照エラー修正版) ---
...
window.stopLiveChat = function() {
    ...
    ['embedded', 'simple-tts'].forEach(ctx => {
        ...
        if(btn) {
             btn.innerHTML = "<span>📷</span> カメラで見せて質問";
             btn.style.backgroundColor = (ctx === 'simple-tts') ? "#009688" : "#66bb6a";
        }
    });
...
window.startAnalysis = async function(b64) {
    ...
    try {
        // ★修正: モデルは gemini-2.5-pro を使用 (サーバー側で処理)
...
window.unlockNextHint = function(level, cost) {
    ...
    window.showHintText(level); renderHintUI();
};
...
window.handleFileUpload = async (file) => {
    ...
    reader.onload = async (e) => {
        ...
        cropImg.onload = async () => {
            ...
            window.initCustomCropper(); // ★修正: window.付きで呼び出し
        };
