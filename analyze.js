// --- analyze.js (完全版 v276.5: ファイル選択イベント修正・関数配置最適化版) ---
...
// ==========================================
// 2. 重要機能: ファイルアップロード & クロップ
// ==========================================
// ※読み込みエラーを防ぐため、最上部に配置

window.handleFileUpload = async function(file) {
    console.log("[Upload] handleFileUpload called with:", file);
    
    // 強制的に分析中フラグを解除（リトライ可能にする）
    window.isAnalyzing = false;
    ...
    if (!canvas) {
        console.error("[Upload] Error: crop-canvas element not found");
        return;
    }
    ...
    reader.onload = async (e) => {
        ...
        cropImg.onload = async () => {
            console.log("[Upload] Image loaded successfully:", cropImg.width, "x", cropImg.height);
            
            // 画面サイズに合わせて初期クロップ領域を設定
...
function initCustomCropper() {
    ...
    const canvas = document.getElementById('crop-canvas');
    const MAX_CANVAS_SIZE = 2500; // メモリ節約のため制限
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // イベントリスナーの重複登録を防ぐため、一度削除などはできないが、
    // DOMContentLoadedは一度しか走らない前提で設定する。
    
    if(camIn) {
        console.log("[System] Camera input found");
        camIn.onchange = function(e) {
            console.log("[Event] Camera input changed");
        ...
        console.error("[System] hw-input-camera not found");
    }
    
    if(albIn) {
        console.log("[System] Album input found");
        albIn.onchange = function(e) {
            console.log("[Event] Album input changed");
            if(e.target.files && e.target.files[0]) {
                window.handleFileUpload(e.target.files[0]); 
            }
            e.target.value = ''; 
        ...
        console.error("[System] hw-input-album not found");
    }
...
function startAlwaysOnListening() {
    ...
    continuousRecognition.onresult = async (event) => {
        // ネル先生がしゃべっている間は認識結果を無視（エコーキャンセル簡易版）
        ...
        try {
            ...
            if(res.ok) {
                const data = await res.json();
                // メッセージ表示＆読み上げ
                // awaitをつけることで、読み上げが終わるまで待つ（updateNellMessageがPromiseを返す前提）
    ...
    // 初回開始
    try { continuousRecognition.start(); } catch(e) { console.log("Rec start failed (user gesture needed?)", e); }
}
...
// updateNellMessage の微修正 (Promiseを返すように明示的に確認)
// (analyze.js内の既存関数定義を上書き)
window.updateNellMessage = async function(t, mood = "normal", saveToMemory = false, speak = true) {
    if (liveSocket && liveSocket.readyState === WebSocket.OPEN && currentMode !== 'chat') {
        speak = false;
    }
